...
CREATE POLICY "Users read their face videos" ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'face-videos' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "Users create their face videos" ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'face-videos' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "Users update their face videos" ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'face-videos' AND (storage.foldername(name))[1] = auth.uid()::text)
WITH CHECK (bucket_id = 'face-videos' AND (storage.foldername(name))[1] = auth.uid()::text);
